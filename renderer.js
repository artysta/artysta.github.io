...
const renderFooterTemplate = (commits, color) => {
        if (commits[0] != undefined) {
            ...
            ${renderLine(color)}
            <div class="footer">
                ${`Last edited at ${commits[0].commit.committer.date.replace('T', ' ').replace('Z', '')} UTC | <a href="${commits[0].html_url}" target="_blank" class="url-${color}-dark no-underline">${commits[0].sha}</a>`}
            </div>
        `
    }
...
const renderSection = (wrapper, section, color) => {
    ...
    if (section.hasOwnProperty('items')) {
        ...
        section.items.forEach(item => {
            ...
            li.innerHTML = `
                ${item.url !== '' ? `<a href="${item.url}" target="_blank" class="url-${color}-dark no-underline">${item.title}</a>` : item.title}
                <ul>
                    ${(item.items2 && item.items2.length > 0) ? item.items2.map(item2 => `
                        <li>${item2.url !== '' ? `<a href="${item2.url}" target="_blank" class="url-${color}-dark no-underline">${item2.value}</a>` : item2.value}</li>
                    `).join('') : ''}
                </ul>
            `;
            ul.appendChild(li);
